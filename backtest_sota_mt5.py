...
class SimManager:
    ...
    def close_trade(self, ticket, exit_price, reason):
        pos = self.positions.pop(ticket)
        # PNL = (PriceDiff) * Lots * ContractSize (100 for XAU)
        pnl = (exit_price - pos.entry) * pos.side * 100.0 * lots_multiplier_safeguard(pos, exit_price)
        # Wait, the lots are already scaled to risk. One lot = 100 oz.
        # If I buy 0.01 lots, I buy 1 oz. 
        # Actually in MT5, 1 lot of Gold is usually 100 oz. 0.01 lot is 1 oz.
        # PNL = (PriceDiff) * lots * 100
        pnl = (exit_price - pos.entry) * pos.side * 100.0 * getattr(pos, 'lots', 0.01)
...
class SOTABacktester:
    ...
    def run(self, df: pd.DataFrame, initial_balance=10000.0, max_positions=3):
        sim = SimManager(initial_balance)
        n = len(df)
        lookback = self.seq_len + 50
        ...
        for i in range(lookback, n):
            ...
            for tid in tickets:
                ...
                if pos.side > 0:
                    if current_bar['low'] <= pos.sl:
                        sim.close_trade(tid, pos.sl, "SL")
                        continue
                    if current_bar['high'] >= pos.tp:
                        sim.close_trade(tid, pos.tp, "TP")
                        continue
                else:
                ...
                upd = self.planner.manage_open(pos, window, i)
                if upd.close_reason == "time":
                    sim.close_trade(tid, current_bar['close'], "TIME")
            ...
            if len(sim.positions) < max_positions:
                try:
                    feat_df = build_live_features(window, self.features)
                    sig, conf, pb, ps, ph = self.predict(feat_df)
                    
                    if sig != 0:
                        plan = self.planner.build_plan(sig, conf, feat_df, sim.balance, 0.05)
                        if not plan.skip:
...
def main():
    ...
    tester.run(df, max_positions=Settings.MAX_POSITIONS)
