#!/usr/bin/env bash
# Run the full SOTA signal-generation pipeline end-to-end.
#
# Assumes you already have Dukascopy M1 OHLCV at:
#   train_pipeline/data/xauusd_m1.csv
#
# Output artifacts:
#   train_pipeline/data/xauusd_m1_synmicro.csv       # OHLCV + synthetic microstructure
#   train_pipeline/data/xauusd_m1_synmicro_tb.csv    # + triple-barrier labels
#   train_pipeline/models_sota/patchtst_primary.pt   # primary model
#   train_pipeline/models_sota/meta_filter.txt       # meta filter
#   train_pipeline/models_sota/sota_config.json
set -euo pipefail

DATA_DIR=${DATA_DIR:-train_pipeline/data}
OUT_DIR=${OUT_DIR:-train_pipeline/models_sota}
RAW=${RAW:-$DATA_DIR/xauusd_m1.csv}
SM=$DATA_DIR/xauusd_m1_synmicro.csv
TB=$DATA_DIR/xauusd_m1_synmicro_tb.csv
